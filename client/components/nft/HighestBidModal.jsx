...
const HighestBidModal = ({ closeBidNFT, auction }) => {
	...
	const formik = useFormik({
		...
		onSubmit: (values) => {
			publicRequest.post("bid", { auctionId: auction._id, bidder: user.walletAdress, value: values.bid })
				.then(res => {
					console.log(res)
				})
				.catch(err => {
								...
								<div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
									<div className="sm:flex sm:items-start">
										...
										<div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
											...
											<div className="flex py-2">
												Current highest bid : {auction.winningBid == null ? auction.startingPrice : auction.winningBid.value} ETH
											</div>
											<div className="mt-2">
												<input
												...
												/>
												{formik.touched.bid && formik.errors.bid ? <div className="text-red-500">{formik.errors.bid}</div> : null}
											</div>
										</div>
									</div>
								</div>
